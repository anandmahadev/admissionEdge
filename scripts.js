const AdmissionEdge = (() => {
  ...
  const initGallery = () => {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      const filtered = activeFil === 'all' ? posters : posters.filter((p) => p.cat === activeFil);
      if (e.key === 'ArrowLeft') { lbIdx = (lbIdx - 1 + filtered.length) % filtered.length; openLightbox(filtered); }
      if (e.key === 'ArrowRight') { lbIdx = (lbIdx + 1) % filtered.length; openLightbox(filtered); }
    });
